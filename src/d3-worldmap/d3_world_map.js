...
export default function D3WorldMap(props) {
    ...
    // useEffect(() => {
    //     console.log('World Map Length : ', geographies.length);
    //     if (geographies.length === 177) {
    //         setMapLoad(!mapLoad);
    //     }
    // }, [geographies])
    ...
    //     let updatedMapData = [];
    //     if (mapLoad === true) {
    //         geographies.map((mapData, i) => (
    //             data.map((d) => {
    //                 // console.log('mapData.properties : ', mapData.properties);
    //                 if (mapData.properties && d['Country Name'] === mapData.properties.name) {
    //                     mapData.properties['poppulation'] = d['2020'];
    //                 }
    //             })
    //         )

    //         )
    //         setGeographies(updatedMapData);
    //         console.log('updatedMapData : ', updatedMapData);
    //     }
    // }, [mapLoad])
    ...
    const makeToolTip = (data, countryIndex) => {
        return (
            <>
                <span className="toolTipHeader">Country Name  :</span> {data.properties.name} 
                <br />
                <span className="toolTipHeader">Total number of Suppliers :</span> {countryIndex}
                <br />
                <span className="toolTipHeader">Total number of Firms :</span> {countryIndex * 10}
    ...
    return (
        <div className="rootDiv">
            <svg width={800} height={450} viewBox="0 0 900 550">
                <g className="countries">
                    {
                        geographies.map((d, i) => (
                            <Tooltip title={makeToolTip(d,i)} arrow>
                                <path
                                    key={`path-${i}`}
